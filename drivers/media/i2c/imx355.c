...
static const struct imx355_clk_params imx355_clk_params[] = {
	{
		...
		.pll_vt_mpy =  0,	/* 19.2 / 2 * 120 = 1152 MHz NJH but we don't use this PLL!? */
		.pll_op_mpy = 75,	/* 19.2 / 2 * 75  = 720 MHz */
	},
	{
		.ext_clk = 24000000,
		.extclk_freq = 0x1800,	/* 24.0 MHz */
		.pll_vt_mpy =  0,	/* 24.0 / 2 * 96  = 1152 MHz NJH but we don't use this PLL!? */
		      ...
		      struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		*framefmt = fmt->format;
		...
		pixel_rate = link_freq_menu_items[mode->link_freq_index] * 2 * imx355->hwcfg->num_data_lanes;
...
static int imx355_identify_module(struct imx355 *imx355)
{
	...
	//strncpy(imx355->sd.name, "imx219", sizeof(imx355->sd.name));
	return 0;
}
...
static int imx355_init_controls(struct imx355 *imx355)
{
	...
	pixel_rate = IMX355_LINK_FREQ_DEFAULT * 2 * imx355->hwcfg->num_data_lanes;
	do_div(pixel_rate, 10);
...
static const struct of_device_id imx355_dt_ids[] = {
	{ .compatible = "sony,imx355" },
	{ /* sentinel */ }
};

MODULE_DEVICE_TABLE(of, imx355_dt_ids);
MODULE_DEVICE_TABLE(acpi, imx355_acpi_ids);

static struct i2c_driver imx355_i2c_driver = {
	.driver = {
		.name = "imx355",
		.of_match_table	= imx355_dt_ids,
