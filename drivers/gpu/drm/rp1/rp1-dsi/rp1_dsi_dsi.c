...
static u64 dphy_get_div(u64 refclk, u64 vco_freq, u32 *ptr_m, u32 *ptr_n)
{
	/*
	...
	static const u64 REF_DIVN_MAX = 40000000;
	static const u64 REF_DIVN_MIN =  5000000;
	unsigned int n, best_n, best_m;
	u64 best_err = vco_freq;

	for (n = 1 + refclk / REF_DIVN_MAX; n * REF_DIVN_MIN <= refclk && n < 100; ++n) {
		u64 half_m = (n * vco_freq + refclk) / (2 * refclk);

		if (half_m < 150) {
			u64 f = (2 * half_m * refclk) / n;
	...
	if (best_err >= (vco_freq >> 6))
		return 0;

	*ptr_n = best_n;
	*ptr_m = best_m;
	return (best_m * refclk) / best_n;
}
...
void rp1dsi_dsi_setup(struct rp1_dsi *dsi, struct drm_display_mode const *mode)
{
	...
	u32 lane_bps = clamp((500u * bpp * (u32)min(mode->clock, 200000)) / dsi->lanes,
			     40000000u, 750000000u) * 2u;
	...
	DSI_WRITE(DSI_CLKMGR_CFG,
		  (RP1DSI_TO_CLK_DIV << 8) |
		  max(2u, 1u + lane_bps / (8 * RP1DSI_ESC_CLK_MAX)));
	...
	/* Init PHY */
	dphy_init(dsi, rp1dsi_refclk_freq(dsi), lane_bps);
